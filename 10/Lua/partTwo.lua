...
stackfactory.meta = { __index = stackfactory.class }
...
---@class recursive
---@field pos coordinate
---@field type boolean

local function dfs(position)
    ---@type stack
    local stack = stackfactory:new()
    stack:push(
    ---@type recursive
        { pos = position, type = true }
    )
    local count = 0
                ...
                for _, dir in ipairs { coordinatefactory:new(-1, 0), coordinatefactory:new(0, 1), coordinatefactory:new(1, 0), coordinatefactory:new(0, -1) } do
                    ...
                    if 1 <= newpos.x and newpos.x <= row and 1 <= newpos.y and newpos.y <= col then
                        if not seen[newpos.x][newpos.y] and map[newpos.x][newpos.y] - map[pos.x][pos.y] == 1 then
                            stack:push({ pos = newpos, type = false })
                            stack:push({ pos = newpos, type = true })
                        end
                    end
                end
            end
